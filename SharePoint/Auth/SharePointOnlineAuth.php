...
class SharePointOnlineAuth extends \SoapClient {
	...
	protected function configureAuthCookies($location) {
        ...
        // Query Microsoft Services To Find Which Authentication Service To Use - Will Determine Adfs, MSOL or SPLA
        $hrd_result = json_decode($this->hrdCurl($login));
        if( $hrd_result->NameSpaceType == 'Federated' ) {
            print('<div>awww crap - this is adfs auth! PANIC PANIC</div>');
        }
        else {
   ...
   /**
    * Home Realm Discovery Request - Queries MS Services To Determine Their Home Realm.
    * Used to find the federation authentication provider.
    */
    protected function hrdCurl($login) {
        $MSHRD_URL = 'https://login.microsoftonline.com/GetUserRealm.srf';

        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL,$MSHRD_URL);
        curl_setopt($ch,CURLOPT_POST,1);
        curl_setopt($ch,CURLOPT_POSTFIELDS,  array('handler' => '1', 'login' => $login));
